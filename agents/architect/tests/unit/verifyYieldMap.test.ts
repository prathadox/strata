...
// Fail-fast guard: if the schema drifts, surface it at module load.
const fixtureCheck = YieldMapSchema.safeParse(BASE_MAP);
if (!fixtureCheck.success) {
  throw new Error(`BASE_MAP is not a valid YieldMap: ${JSON.stringify(fixtureCheck.error.flatten())}`);
}

async function signMapWith(privateKey: `0x${string}`, unsignedMap: YieldMap): Promise<YieldMap> {
  const account = privateKeyToAccount(privateKey);
  // Set the publisher address first, then sign - so the signature covers the correct address.
  const withAddress: YieldMap = {
    ...unsignedMap,
    publisher: { ...unsignedMap.publisher, address: account.address },
    signature: ''
  };
...
describe('verifyYieldMap', () => {
  it('resolves without throwing for a valid signed map', async () => {
    ...
    await expect(verifyYieldMap(signed)).resolves.toBeUndefined();
  });
  ...
  it('rejects when a field is tampered after signing', async () => {
    ...
    await expect(verifyYieldMap(tampered)).rejects.toThrow(
  ...
  it('rejects when publisher.address does not match the signing key', async () => {
    ...
    await expect(verifyYieldMap(wrongPublisher)).rejects.toThrow(
      'signature does not recover to publisher.address'
    );
  });
  ...
  it('resolves in strict mode when expectedSigner matches publisher', async () => {
    ...
    const signed = await signMapWith(pk, BASE_MAP);
    await expect(verifyYieldMap(signed, account.address)).resolves.toBeUndefined();
  });
  ...
  it('rejects in strict mode when expectedSigner differs from actual signer', async () => {
    ...
    await expect(verifyYieldMap(signed, accountB.address)).rejects.toThrow(
