...
async function main(): Promise<void> {
  ...
  const stratCid = await pinJsonToPinata(JSON.stringify({ kind: 'strategy', text: strategy }), apiKey);
  const methCid = await pinJsonToPinata(JSON.stringify({ kind: 'hedge-methodology', text: methodology }), apiKey);
